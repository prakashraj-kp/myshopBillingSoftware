...
import {lastestOrder} from "../../Service/OrderService";
...
const OrderHistory =()=>{
   ...
   useEffect(() => {
    const fetchOrders = async () => {
        try {
            const response = await lastestOrder();
                        ...
                        <tr key={order.orderId}>
                            ...
                            <td>
                                <span className={`badge ${order.paymentDetails?.status==="COMPLETED"?
                                    "bg-success":"bg-warning text-dark"}`}>{order.paymentDetails?.status||"PENDING"}</span>
                            </td>
